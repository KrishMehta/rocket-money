...
type ChatEntry = { role: 'user' | 'assistant' | 'ai'; content: string };
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { message, conversation } = parsedBody as {
    message?: string;
      ...
      ? conversation
          .filter(
            (entry) =>
              ...
              (entry.role === 'user' || entry.role === 'assistant' || entry.role === 'ai')
          )
...
async function buildFinancialContext(supabase: any, userId: string) {
  ...
  try {
    ...
    const getCategoryName = (tx: any) => {
      return tx.user_category_name || tx.plaid_primary_category || 'Uncategorized';
    };
    ...
    const expenses = transactions.filter((tx: any) => {
      ...
      const isCreditCardPayment =
        /payment.*(american express|amex|chase|discover|capital one|citibank|card)/i.test(
          fullName
        ) ||
    ...
    const expenseRecurring = recurring.filter((item: any) => item.transaction_type === 'expense');
    ...
    if (context.recurring.subscriptionCount > 0 && context.spending.totalSpending30 > 0) {
      ...
      insights.push(`Recurring charges represent ${recurringPercent.toFixed(1)}% of monthly spend.`);
    }
...
function summarizeContextForPrompt(context: any) {
  if (!context) {
  ...
  if (context.recurring.upcomingCharges.length) {
    const upcomingText = context.recurring.upcomingCharges
      .map(
        (charge: any) =>
          `${charge.name} ${formatCurrency(charge.amount)} due ${charge.next_due_date || 'soon'}`
      )
      .join('; ');
...
function extractMessageText(completion: any) {
